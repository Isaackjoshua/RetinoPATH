"""Phase 6 (3-class R0/R1/R2) — 4-way TTA + patient aggregation on the test set.

Loads the 5 fold checkpoints (224px, 3-class head), runs 4-way TTA over the
3-class-filtered test set (eval transform = build_transform('val') so it matches
exactly how P6 validated), ensembles across folds, then reports image-level and
patient-pooled (mean/max) metrics. Mirrors phase3_tta_eval.py but for 3 classes
and with the R3A-patient cohort filter.
"""
...
from p6_cohort import filter_r0r2_patients

INPUT_SIZE, NUM_CLASSES, N_FOLDS = 224, 3, 5
CVDIR = 'output_dir/phase6_3class_cv'
CLASSES = ['R0', 'R1', 'R2']
dev = 'cuda'
GRADE = {'R0': 0, 'R1': 1, 'R2': 2, 'R3A': 3}

# ── 3-class test cohort (same filter as the pilot notebook) ───────────────────
df = pd.read_csv('labels/splits.csv')
df['grade_int'] = df['retinopathy'].map(GRADE)
df = filter_r0r2_patients(df)                      # drop patients whose max grade is R3A
df_te = df[df['split'] == 'test'].reset_index(drop=True)
test_paths = df_te['image_path'].values
print(f'Test set (3-class): {len(df_te)} images | {df_te["code"].nunique()} patients '
      f'| per-class images {df_te["retinopathy"].value_counts().reindex(CLASSES).tolist()}')

# ── Base eval transform identical to the pilot's eval_transform ───────────────
...
for how in ['mean', 'max']:
    ...
    if how == 'mean':
        print('   PtMean confusion (rows=true R0/R1/R2):')
        for i, c in enumerate(CLASSES):
            print('     ', c, m['cm'][i].tolist())

with open(f'{CVDIR}/phase6_tta_results.json', 'w') as f:
    json.dump(results, f, indent=2, default=float)
print(f'\nSaved {CVDIR}/phase6_tta_results.json')
